  ...
  return (
    <nav
      class={`fixed w-full z-50 transition-all duration-300 ${
        scrolled()
          ? "bg-[#21204F]/95 dark:bg-[#21204F]/95 backdrop-blur-lg shadow-lg h-16"
          : "bg-white dark:bg-[#21204F] h-16"
      }`}
    >
      <div class="max-w-7xl mx-auto px-4 h-full">
        <div class="flex justify-between items-center h-full">
          {/* Logo */}
          <div class="flex-shrink-0 transform hover:scale-105 transition-transform duration-200">
            <a href="/" class="flex items-center">
              <img src={logo} class="h-8" alt="Kaizen Notes" />
            </a>
          </div>
          ...
          <div class="hidden lg:flex items-center space-x-8">
            ...
            <button
              onClick={toggleTheme}
              class="p-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#2d2c5e] transition-colors duration-200"
            ...
            {navContent.navLinks.map((link) => (
              <a
                ...
                class={`text-gray-600 dark:text-white hover:text-gray-900 dark:hover:text-gray-300 transition-colors duration-200 relative group text-sm font-medium ${
                  currentPath() === link.path ? "text-gray-900 dark:text-white" : ""
                }`}
              >
                {link.name}
                <span class={`absolute -bottom-1 left-0 w-full h-0.5 bg-[#853232] transform origin-left scale-x-0 transition-transform duration-300 group-hover:scale-x-100 ${
          ...
          <div class="lg:hidden flex items-center space-x-4">
            ...
            <button
              ...
              class="p-2 rounded-lg text-gray-600 dark:text-white hover:bg-gray-100 dark:hover:bg-[#2d2c5e] transition-colors duration-200 focus:outline-none"
        ...
        >
          <div class="bg-white dark:bg-[#2d2c5e] shadow-xl rounded-b-2xl mx-4 p-4 border border-gray-200 dark:border-gray-700">
            <div class="space-y-3">
              {navContent.navLinks.map((link) => (
                <a
                  href={link.path}
                  ...
                  class={`block px-4 py-2 text-gray-600 dark:text-white hover:bg-gray-100 dark:hover:bg-[#21204F] rounded-lg transition-colors duration-200 text-sm ${
                    currentPath() === link.path ? "bg-gray-100 dark:bg-[#21204F]" : ""
                  }`}
                >
                  {link.name}
                </a>
